...
if (androidSpecific) {
  ...
  // Populate picker.
  populatePicker();
	var anim_maximize = Titanium.UI.createAnimation({height:260,duration:250});
	//Ti.API.info('$currentPlaylistPicker = ' + JSON.stringify($currentPlaylistPicker));
	pickerView.animate(anim_maximize);
}
...
function close(_callbackParams) {
  ...
  if (_.isFunction(args.onDone)) {
	...
	if (OS_IOS) {
	  	var anim_minimize = Titanium.UI.createAnimation({height:1,duration:250});
		//Ti.API.info('$currentPlaylistPicker = ' + JSON.stringify($currentPlaylistPicker));
		pickerView.animate(anim_minimize, function() {
			cleanup();
		});
	} else {
		cleanup();
